...
# check names of vars to be excluded
  if (!is.null(exclude)) {
    ...
    if (any(is.na(exclude.cols))) {
      stop("Unrecognized variable(s) in exclude parameter for uniques: ",
        paste(exclude[is.na(exclude.cols)], collapse = ", "),
        call. = FALSE
      )
    }
  }
...
# extract uniques from the original data
  ...
  return(list(
    replications = rm.Syn, no.uniques = no.uniques,
...
sdc <- function(
    object, data, label = NULL, rm.replicated.uniques = FALSE,
  ...
  if (object$m == 1) {
    if (!is.null(recode.vars)) {
      ...
      for (i in cols) {
        j <- match(i, cols)
        recoded <- bottom.top.recoding(
          object$syn[, i], bottom.top.coding[[j]][1],
  ...
  if (object$m > 1) {
    if (!is.null(recode.vars)) {
      ...
      for (k in 1:object$m) {
        ...
        for (i in cols) {
          j <- match(i, cols)
          recoded <- bottom.top.recoding(
            object$syn[[k]][, i], bottom.top.coding[[j]][1],
            bottom.top.coding[[j]][2], recode.exclude[[j]]
          )
    ...
    if (!is.null(label)) {
      object$syn <- mapply(cbind.data.frame,
        flag = label,
...
bottom.top.recoding <- function(x, bottom, top, exclude = NULL) {
  below <- which(x < bottom & !x %in% exclude)
  no.below <- length(below)
  above <- which(x > top & !x %in% exclude)
  no.above <- length(above)
  x[below] <- bottom
  x[above] <- top
...
# without.ext <- strsplit(file,"\\.")[[1]][1]      #! will include path
  ...
  if (filetype == "SPSS") {
    if (m == 1) {
      f1 <- paste0(filename, ".sps")
      f2 <- paste0(filename, ".txt")
    } else {
      f1 <- paste0(filename, "_", 1:m, ".sps")
      f2 <- paste0(filename, "_", 1:m, ".txt")
    }
    for (i in 1:m) {
# write.foreign(object$syn[[i]], codefile=f1, datafile=f2, package=filetype, ...)
      write.syn.SPSS(object$syn[[i]],
        codefile = f1[i], datafile = f2[i],
...
# !### check why default convert.factors="labels" cuts the names
    ...
    if (m == 1) {
      f1 <- paste0(filename, ".sas")
      f2 <- paste0(filename, ".txt")
    } else {
      f1 <- paste0(filename, "_", 1:m, ".sas")
      f2 <- paste0(filename, "_", 1:m, ".txt")
    }
    for (i in 1:m) {
...
write.syn.SPSS <- function(
    df, datafile, codefile, varnames = names(df),
    data.labels = NULL, ...) {
  ...
  for (i in 1:ncol(dfn)) {
    if (!is.null(vallabels[[varnames[i]]])) {
      dfn[, i] <- mapvalues(dfn[, i],
        from = names(vallabels[[varnames[i]]]),
