...
summary.synds <- function(
  ...
  if (object$m == 1) {
    ...
    for (i in 1:object$m) {
      zall[[i]] <- lapply(object$syn[[i]], summary,
...
# Return the complete data analyses as a list of length m
  object <- list(
    call = call, mcoefavg = allcoefvar$mcoefavg,
...
# Do the repeated analysis, store the result without data
  for (i in 1:data$m) {
    if (data$m == 1) {
      fit <- polr(formula, data = data$syn, Hess = TRUE, ...)
    } else {
      fit <- polr(formula, data = data$syn[[i]], Hess = TRUE, ...)
    }
    ss <- summary(fit)
    analyses[[i]] <- ss
  }
...
# Do the repated analysis, store the result without data
  for (i in 1:data$m) {
    if (data$m == 1) {
      fit <- multinom(formula, data = data$syn, Hess = TRUE, ...)
    } else {
      fit <- multinom(formula, data = data$syn[[i]], Hess = TRUE, ...)
    }
    ss <- summary(fit)
    analyses[[i]] <- list(coefficients = cbind(
      as.vector(t(ss$coefficients)),
      as.vector(t(ss$standard.errors)),
      as.vector(t(ss$coefficients) / t(ss$standard.errors))
    ))
    dd <- dimnames(t(ss$coefficients))
    dimnames(analyses[[i]]$coefficients) <- list(
...
checkcomplete <- function(vars, formula, vs, method) {
  ...
  if (any(!inform %in% vars)) {
    stop("Variable(s) in formula (model to be fitted) are not in synthetic data: ",
      paste(inform[!inform %in% vars], collapse = ", "),
      call. = FALSE
    )
  }
...
summary.fit.synds <- function(object, population.inference = FALSE, msel = NULL,
                              real.varcov = NULL, incomplete = NULL, ...) { # df.residual changed to df[2] because didn't work for lm
  ...
  if (!is.null(real.varcov)) {
    vars <- diag(real.varcov)
  } else {
    vars <- object$mvaravg * k / n
  } # mean of variances (over m syntheses) * adjustment
...
#---
  if (population.inference == TRUE) {
    if (incomplete == TRUE & m == 1) {
      cat(
        "Warning: You have selected population inference when your dependent variable is not synthesised",
...
#---
  ...
  res <- list(
    call = object$call, proper = object$proper,
...
print.compare.fit.synds <- function(x, print.coef = x$print.coef, ...) {
  ...
  if (!is.null(x$lack.of.fit)) {
    ...
    if (x$incomplete == FALSE) {
      cat("with a chi-squared test with ",
        x$ncoef, " degrees of freedom.\n",
        sep = ""
      )
    } else {
      cat("with an F distribution with ", x$ncoef, " and ",
...
print.utility.gen <- function(x, digits = NULL, zthresh = NULL,
  ...
  if (x$method == "logit") {
    ...
    if (converged == FALSE) {
      cat("Warning: Logistic model did not converge in ", x$maxit,
...
Warning: null pMSE resamples failed to split ", x$nnosplits[1], " time(s) from ", x$nnosplits[2],
  ...
  if (x$m == 1) {
    cat("\nSelected utility measures\n")
    tabres <- c(
      round(x$pMSE, digits), round(x$S_pMSE, digits),
...
summary.out <- function(z, digits = max(3L, getOption("digits") - 3L), ...) {
  ...
  for (i in seq_len(nv)) {
    ...
    if (is.matrix(sms)) {
      ...
      pad1 <- wid - wcn - pad0
      cn <- paste0(substring(blanks, 1L, pad0), cn, substring(
        blanks,
        1L, pad1
      ))
