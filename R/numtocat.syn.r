  ...
  if (!is.null(numtocat)) {
    if (is.numeric(numtocat)) {
      if (!all(numtocat %in% 1:length(data))) {
        stop("Column indices must be between 1 and ",
          length(data), ".",
          sep = "", call. = FALSE
        )
      }
...
# checks on catgroups
  if (length(catgroups) == 1) {
    catgroups <- rep(catgroups, length(numtocat))
...
# checks on cont.na
  if (!is.null(cont.na)) {
    ...
    if (!all(names(cont.na) %in% numtocat)) {
      stop("Variable(s): ",
  ...
  if (print.flag == TRUE) {
    cat("Variable(s) ", paste(numtocat, collapse = ", "),
      " grouped into categories.\n",
      sep = ""
    )
  }
  breaks <- vector("list", length(varnos))
  names(breaks) <- numtocat
  levels <- vector("list", length(varnos))
  names(levels) <- numtocat
