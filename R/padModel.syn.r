...
# to tell sampler.syn how to select columns
  ...
  categories <- data.frame(
    yes.no.categorical = factor(rep(FALSE, nvar),
      levels = c("TRUE", "FALSE")
    ),
...
# pred.with.cart <- method %in% c("ctree", "ctree.proper", "cart", "cart.proper", "collinear", "satcat") #!BN to check if collinear needed
  ...
  pred.with.cart <- !method %in% c(
    "norm", "normrank", "logreg", "lognorm",
    "polr", "polyreg", "cubertnorm", "sqrtnorm"
  ) # !GR050318
  for (j in 1:nvar) {
...
# a predictor in a non-CART model, 0 otherwise                          #!BN1605
      predictor.matrix <- cbind(predictor.matrix, matrix(0,
        ncol = n.dummy, # !BN1605
...
# when data are not missing and labels columns
      ...
      categories <- rbind(
        categories,
        data.frame(
          yes.no.categorical = rep(FALSE, n.dummy),
