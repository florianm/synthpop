...
#--- Assign optional parameters (...) to appropriate synthesising function
  ...
  meth.with.opt <- paste(c(
    "cart", "cartbboot", "ctree", "survctree", "polyreg",
  ...
  if (any(args.err)) {
    stop("Unknown optional parameter(s): ",
...
#---
  ...
  if (m > 0) {
    if (models) {
      fits <- rep(list(setNames(
        vector("list", length(p$method)),
        names(p$method)
      )), m)
    }
...
## to reorder any dummies for grouped variables
        ...
        if (print.flag == TRUE) {
          if (length(rest.visit.sequence) > 0 &&
            ...
            if (ordmethod[1] == "catall" && !is.null(mth.args) &&
              ...
              cat(
                "Synthesis made differentially private with parameter epsilon of ",
        ...
        x <- p$data[, grouped]
        if (!(ordmethod[1] %in% names(mth.args))) {
          fun.args <- NULL
        } else {
          fun.args <- mth.args[[ordmethod[1]]]
        }
        f <- paste("syn", ordmethod[1], sep = ".")
        synfun <- do.call(f, args = c(list(
          x = x, k = k,
          proper = proper
        ), fun.args))
...
# get optional parameters for theMethod if they are provided
          if (!(theMethod %in% names(mth.args))) {
            fun.args <- NULL
          } else {
...
# if(theMethod!="collinear"){
              ...
              if (theMethod == "survctree") {
                if (p$event[j] == -1) {
                  yevent <- rep(1, length(y))
                } else {
                  yevent <- p$data[ya, p$event[j]]
                }
                survres <- do.call(f, args = c(
                  list(
                    y = y, yevent = yevent,
