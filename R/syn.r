...
# check problematic characters in varaiable names
  ...
  if (any(has_space == 3)) {
    stop(paste(
      "Your data have numeric variable(s) with missing values with names that include spaces:\n  ",
      paste0("`", paste(obs.vars[has_space == 3], collapse = "`, `"), "`"),
      "\nThese should be renamed for synthpop to work correctly."
    ), call. = FALSE)
  }
  ...
  if (any(has_bad_char)) {
    cat(
      "WARNING: Some variable names include special characters",
...
# if visit sequence includes variable names change them into column indecies
  if (is.character(visit.sequence)) {
    ...
    if (any(is.na(nametoind))) {
      ...
      visit.sequence <- nametoind
    }
  } else if (!all(visit.sequence %in% 1:length(data))) {
    stop("Column indices in visit.sequence must be between 1 and ",
      length(data),
      sep = "", call. = FALSE
    )
  }
...
# if numtocat is numeric change to column names and check names if not
    if (is.numeric(numtocat)) {
      if (!all(numtocat %in% 1:length(data))) {
        stop("Column indices in visit.sequence must be between 1 and ",
          length(data),
          sep = "", call. = FALSE
        )
    ...
    } else if (!all(numtocat %in% names(data))) {
      stop("numtocat variable(s): ",
        paste(numtocat[!numtocat %in% names(data)], collapse = ", "), " are not in data.",
        sep = "", call. = FALSE
      )
    }
...
# make numtocat object incorporating some checks
    numtocat.obj <- numtocat.syn(data,
      numtocat = numtocat, cont.na = cna,
...
# replace data with categorised with orig vars at end
    ...
    if (length(method) == 1) {
      if (method == "parametric") {
        stop("'parametric' not available, when numtocat is used, methods must be specified in detail.\n", call. = FALSE)
      } else {
        method <- rep(method, length(data) - length(numtocat))
      }
    }
...
# checks on methods for numtocat variables
    if (any(method[numtocat.obj$ind] %in%
      ...
      stop("Method(s) (", paste(method[numtocat.obj$ind][nummth], collapse = ", "),
        ...
        ")\nunsuitable for categorical data.",
        sep = "", call. = FALSE
      )
    }
...
# remove any visitSequnce members outside 1:nvar
    if (any(!(vis %in% 1:nvar))) {
      cat("Element(s): {", paste(vis[!(vis %in% 1:nvar)],
        collapse = ", "
      ), "} of the 'visit.sequence' removed as not valid. No such column.\n\n", sep = "")
...
# remove event indicator(s) from visit.sequence, if present
    ...
    if (!is.null(event) & any(event.in.vis) && method[which(event == vis[event.in.vis])] == "survctree") {
      ...
      if (length(vis) < 2) {
        stop("Visit sequence now of length ",
...
# check that denominator comes before the count for a binomial with denom
    for (j in which(denom[vis] > 0)) {
      ...
      if (j < denom.pos) {
        stop("Denominator ", varnames[denom[vis][j]], " for ",
...
# not already synthesised
    ...
    if (length(vis) > 1) {
      ...
      if (nrow(preddel) > 0) {
        cat(paste("Not synthesised predictor ",
...
#---
    ...
    for (i in passive.idx) {
      ...
      if (is.factor(data.val)) {
        ...
        if (!all(levels(data.val) == levels(passive.val))) {
          stop("Levels of passively created factor ",
      ...
      if (is.numeric(data.val) & any(is.na(data.val))) {
        cat("\nVariable ", varnames[i],
      ...
      if (length(vis) > 1) {
        for (j in vis[-1]) {
          if (has.pred[j]) {
            ...
            } # to prevent a constant values failing
...
# fullNames <- paste("syn", method[active], sep=".") #!GR-29/8/16
      ...
      notFound <- !(fullNames %in% c(
        "syn.bag", "syn.cart", "syn.cartbboot", "syn.collinear",
      ...
      if (any(notFound)) {
        stop(paste(
          ...
          paste(unique(fullNames[notFound]), collapse = ", ")
        ), call. = FALSE)
      }
    }
...
# type checks on built-in  methods
    ...
    for (j in vis) {
      ...
      mj <- method[j]
      mlist <- list(
        m1 = c("logreg", "polyreg", "polr", "ipf", "catall"), # GRBN
...
#  check all integers
        ...
        if (any((data[denom[j]] - y) < 0, na.rm = TRUE)) {
          stop("Variable ", vname, # !!!!!! address missing data issue
...
# check if var has predictors (re-compute it)
    if (sum(pred) > 0) {
      ...
      has.pred <- rep(0, sqrt(length(pred)))
    } # this needed in case pred now has dimension 1

    for (j in vis) {
      if (!has.pred[j] & substr(method[j], 1, 6) != "nested" & is.na(any(match(
        method[j],
      ...
      for (j in vis) { # checks for survival variables
        ...
        if (method[j] == "survctree") {
          ...
          if (is.na((match(event[j], 1:nvar)))) {
            ...
            if (any(is.na(data[, event[j]]))) {
              stop("Missing values in event indicator '", colnames(data)[event[j]],
                "' for survival time '", vname, "'. No data synthesised.",
                call. = FALSE
              )
            }
            ...
            if (is.character(data[, event[j]])) {
              ...
              cat(
                "Value", levels(data[, event[j]])[2], "of event indicator",
            ...
            if (length(tabEI) != 2) {
              ...
              } else if (length(tabEI) == 1 & all(tabEI == 0)) {
                stop("Variable ",
      ...
      if (any(inpred)) {
        ...
        if (length(collout) > 0) {
          for (i in 1:length(collout)) {
            if (print.flag) {
              cat("Variables ", paste(collout[[i]], collapse = ", "),
                " are collinear.",
                sep = ""
              )
            }
            ...
            if (print.flag) {
              cat(" Variables later in 'visit.sequence'\nare derived from ",
                vfirst, ".\n\n",
                sep = ""
              )
            }
...
# get.vars <- lapply(get.vars,function(x) gsub(" ","",x))          # remove spaces
    ...
    if (any(vars.wrong)) {
      stop("Unexpected variable(s) in rules: ",
        paste(vars.in.rules[vars.wrong], collapse = ", "), ".",
        call. = FALSE
      )
    }
...
# the variables they apply to
    ...
    var.in.vis <- lapply(var.position, function(x) {
      if (length(x) == 0) {
        x <- 0
...
# and create list with elements for each variable
  namedlist <- function(x, varnames = colnames(data),
    ...
    if (is.null(x)) {
      ...
      if (any(is.na(x.ind))) {
        stop("Unrecognized variable names in '",
          argname, "': ", paste(names(x)[is.na(x.ind)], collapse = ", "),
          call. = FALSE
        )
      }
...
# change them to column indecies
      if (argname %in% c("denom", "event") & is.character(argname)) {
        ...
        if (any(is.na(denom.ind))) {
          stop("Unrecognized variable(s) provided as ", argname, "(s): ",
            paste(unlist(x)[is.na(denom.ind)], collapse = ", "),
            call. = FALSE
          )
        }
        x <- denom.ind
...
# methods must be given for all columns in the data
  if (length(method) != nvar) {
    stop(
      paste("The length of method (", length(method),
...
# S E M I - C O N T I N O U S  V A R S
  semicont <- namedlist(semicont,
    missval = NA, argname = "semicont",
    argdescription = "semi-continuous"
  )
  cont.na <- namedlist(cont.na,
    missval = NA, argname = "cont.na",
    argdescription = ""
  )
...
# R U L E S  and  R V A L U E S
  rules <- namedlist(rules,
    missval = "", argname = "rules",
    argdescription = ""
  )
  rvalues <- namedlist(rvalues,
    missval = NA, argname = "rvalues",
    argdescription = ""
  )
# S M O O T H I N G
  smoothing <- namedlist(smoothing,
    missval = "", argname = "smoothing",
    ...
    for (j in vartofactor) {
      if (setup$method[j] %in% c(
        ...
        if (nlevel[j] == 2) {
          ...
          setup$method[j] <- default.method[3]
        }
...
# check.method and check.data
  if (sum(predictor.matrix) > 0) { # GR condition added
    ...
    inpred <- ispredictor <- rep(0, sqrt(length(predictor.matrix)))
  }
...
# Print out info on variables not synthesised but used in prediction
  ...
  if (sum(predictor.matrix) > 0) {
    pm <- padMis.syn(
      data, method, predictor.matrix, visit.sequence,
...
#                   nvar, rules, rvalues)
    p <- padModel.syn(
      pm$data, pm$method, pm$predictor.matrix, pm$visit.sequence,
...
# }
  ...
  synall <- sampler.syn(
    p, data, m, syn, visit.sequence, rules, rvalues,
