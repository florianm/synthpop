...
# -> change into one factor with strata names
  if (is.character(strata) & any(!duplicated(strata))) {
    ...
    if (any(is.na(varindex))) {
      stop("Unrecognized variable(s) in strata: ",
        paste(strata[is.na(varindex)], collapse = ", "),
        call. = FALSE
      )
    } else {
...
# check length of strata vector if given as vector; check for missing
    if (length(strata) != nrow(data)) {
      stop(paste("The length of strata index (",
    ...
    if (any(is.na(strata))) {
      stop("Strata indicator cannot have missing values.",
        call. = FALSE
      )
    }
...
# check min number of observations in strata
  ...
  if (smallstrata > 5) {
    stop("In the original data multiple strata have fewer than the recommended\nnumber of observations. We advise that each should have at least ",
  ...
  if (smallstrata > 0) {
    stop("In the original data some strata (",
  ...
  synds.names <- c(
    "call", "m", "syn", "method", "visit.sequence",
  ...
  synds <- rep(synds, m0)
  sel.names <- match(c(
    "call", "m", "predictor.matrix", "proper", "strata.syn",
    "strata.lab", "seed", "numtocat", "catgroups", "models"
  ), synds.names)
  same.by.m <- c(
    "call", "m", "method", "visit.sequence", "predictor.matrix",
  ...
  for (j in 1:m0) {
    synds[[j]]$strata.syn <- sort(factor(sample(stratalev.lab, k,
      replace = TRUE,
      prob = strata.n.obs
    ), levels = stratalev.lab, exclude = NULL))
...
# syn.args$visit.sequence <- c(vs[(vs %in% strata)],vs[!(vs %in% strata)])  #!GR 9/17 move strata to start of visit sequence
    ...
    for (i in 1:nstrata) {
      ...
      syn.args$k <- strata.n.syn[[j]][i]
      names(syn.args$k) <- "strata"
      syn.args$seed <- NA
    ...
    synds[[j]]$predictor.matrix <- eval(parse(text = paste0(
      "list(",
      paste0("synds.ind[[", 1:nstrata, "]]$predictor.matrix", collapse = ", "), ")"
    )))
    synds[[j]]$models <- eval(parse(text = paste0(
      "list(",
      paste0("synds.ind[[", 1:nstrata, "]]$models", collapse = ", "), ")"
    )))
    synds[[j]]$seed <- seed
    synds[[j]][-sel.names] <- eval(parse(text = paste0(
      "Map(rbind, ",
