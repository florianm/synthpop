...
# Check selected variables and make observed and synthetic comparable
  if (!(is.null(vars))) {
    ...
    if (m == 1) {
      if (!all(vars %in% names(object$syn))) {
        stop("Some 'vars' specified not in synthetic data.\n", call. = FALSE)
      } else {
        object$syn <- object$syn[, vars, drop = FALSE]
      }
    } else {
      if (!all(vars %in% names(object$syn[[1]]))) {
        stop("Some 'vars' specified not in synthetic data.\n", call. = FALSE)
      } else {
        object$syn <- lapply(object$syn, "[", vars)
      }
    }
  } else {
    if (m == 1) vars <- names(object$syn) else vars <- names(object$syn[[1]])
    if (!all(vars %in% names(data))) {
      stop("Some variables in synthetic data not in original data.\n", call. = FALSE)
    } else {
      data <- data[, vars]
    } # make data match synthetic
  }
...
# Drop any single value columns
  ...
  if (m == 1) {
    sout <- sapply(object$syn, leneq1)
  } else {
    sout <- sapply(object$syn[[1]], leneq1)
  }
  dout <- dout & sout
  if (any(dout == TRUE) & print.flag) {
    ...
    data <- data[, !dout]
    if (m == 1) {
      object$syn <- object$syn[, !dout, drop = FALSE]
    } else {
      object$syn <- lapply(object$syn, "[", !dout)
    }
  }
...
# Categorical vars: make missing data part of factor
  ...
  for (i in catvars) {
    ...
    if (m == 1) {
      object$syn[, i] <- factor(object$syn[, i])
    } else {
      for (j in 1:m) object$syn[[j]][, i] <- factor(object$syn[[j]][, i])
    }
    if (any(is.na(data[, i]))) {
      data[, i] <- addNA(data[, i])
      if (m == 1) {
        object$syn[, i] <- addNA(object$syn[, i])
      } else {
        for (j in 1:m) object$syn[[j]][, i] <- addNA(object$syn[[j]][, i])
      }
    }
  }
...
# cheking for large models
      ...
      levs[levs == 0] <- 2
      tt1 <- apply(combn(length(levs), 1), 2, function(x) {
        prod(levs[x] - 1)
      })
      if (maxorder == 0) {
        nparams <- 1 + sum(tt1)
      } else {
        tt2 <- apply(combn(length(levs), 2), 2, function(x) {
          prod(levs[x] - 1)
        })
        if (maxorder == 1) {
          nparams <- 1 + sum(tt1) + sum(tt2)
        } else {
          tt3 <- apply(combn(length(levs), 3), 2, function(x) {
            prod(levs[x] - 1)
          })
          if (maxorder == 2) {
            nparams <- 1 + sum(tt1) + sum(tt2) + sum(tt3)
          } else {
            tt4 <- apply(combn(length(levs), 4), 2, function(x) {
              prod(levs[x] - 1)
            })
            if (maxorder == 3) {
              nparams <- 1 + sum(tt1) + sum(tt2) + sum(tt3) + sum(tt4)
            } else {
              tt5 <- apply(combn(length(levs), 5), 2, function(x) {
                prod(levs[x] - 1)
              })
      ...
      if (nparams > max.params) {
        stop("You will be fitting a large model with ", nparams,
...
You can try again, if you really want to, by increasing max.params.\n",
      ...
      } else if (nparams > dim(data)[[1]] / 5) {
        cat(
          "You will be fitting a large model with ", nparams,
...
Have you selected variables with vars?\n"
      ...
      if (aggregate == TRUE) {
        fit <- glm(logit.int,
          data = aggdat, family = "binomial",
          control = list(maxit = maxit), weights = wt
        )
      } else {
        fit <- suppressWarnings(glm(logit.int,
          data = df.prop, family = "binomial",
...
# Get number of parameters that involve synthesised variables
      ...
      if (incomplete == FALSE) {
        km1 <- length(fit$coefficients[!is.na(fit$coefficients)]) - 1
      } # To allow for non-identified coefficients
      else {
        ...
        if (m == 1 || (m > 1 & j == 1)) {
          cat("Expectation of utility uses only coefficients involving synthesised variables: ",
...
# Permutations
    if (!is.null(resamp.method) && resamp.method == "none") {
      S_pMSE <- NA
      ...
      if (m == 1) j <- 1
      if (j == 1 & print.flag) {
        if (print.every == 0 | print.every >= nperms) {
          cat("Running ", nperms, " permutations to get NULL utilities.", sep = "")
        } else {
          cat("Running ", nperms, " permutations to get NULL utilities and printing every ", print.every, "th.", sep = "")
        }
      }
# if (print.flag) cat("\nsynthesis ", j, "   ", sep = "")
      if (print.flag) cat("\nsynthesis ")
      ...
      for (i in 1:nperms) {
        ...
        if (method == "cart") {
          ...
          if (sfit$converged == FALSE & print.flag) {
            cat("Warning: Logistic model did not converge in ",
    ...
    if (!is.null(resamp.method) && resamp.method == "pairs") {
      res.ind <- list(
        pMSE = pMSE, SPECKS = SPECKS, PO50 = PO50, U = U,
...
# end propcalcs
  ...
  if (m == 1) {
    ...
    res <- list(
      call = match.call(), m = m, method = method, tree.method = tree.method,
    ...
    for (j in 1:m) {
      ...
      if (!is.null(resamp.method) && resamp.method == "pairs") {
        if (j == 1 & print.flag) {
          if (print.every == 0 | m * (m - 1) / 2 <= print.every) {
            cat("Simulating NULL pMSE from ", m * (m - 1) / 2, " pair(s).", sep = "")
          } else {
            cat("Simulating NULL pMSE from ", m * (m - 1) / 2, " pairs, printing every ", print.every, "th:\n", sep = "")
          }
...
#---------
  ...
  if (is.null(vars)) {
    stop("You need to set variables with vars parameter.\n", call. = FALSE)
  } else if
  (!(all(vars %in% names(data)))) {
    stop("Unrecognized variable(s) in vars parameter: ",
...
## check table size
    ...
    } else if (i == 1 & table.size > dim(data)[1] / 2 & print.flag) {
      cat("Warning: You are creating tables with ", table.size,
