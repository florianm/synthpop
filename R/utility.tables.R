...
# sort out cont.na to make it into a complete named list
    ...
    object <- list(
      syn = object, m = m, strata.syn = NULL,
      method = syn.method, cont.na = cont.na
    )
    class(object) <- "synds"

    res <- utility.tables.synds(
...
utility.tables.synds <- function(object, data,
  ...
  ord <- order(vno)
  vars <- vars[ord]
  vno <- vno[ord] ## get in right order in syn
...
# check tab.stats (for tabs) and plot.stat (for plots)
  if (is.null(tab.stats)) {
    tab.stats <- c("pMSE", "S_pMSE", "df")
  } else if (!all(tab.stats %in% c(
    "VW", "FT", "JSD", "SPECKS", "WMabsDD", "U", "G", "pMSE", "PO50",
    "MabsDD", "dBhatt", "S_VW", "S_FT", "S_JSD", "S_WMabsDD", "S_G", "S_pMSE", "df", "dfG", "all"
  ))) {
    stop('Parameter tab.stats can only include:
"VW", "FT", "JSD", "SPECKS", "WMabsDD", "U", "G", "pMSE", "PO50",
"MabsDD", "dBhatt", "S_VW", "S_FT", "S_JSD", "S_WMabsDD", "S_G", "S_pMSE", "df", "dfG", "all".\n', call. = FALSE, sep = "")
  }
  if (any(tab.stats == "all")) {
    tab.stats <- c(
      "VW", "FT", "JSD", "SPECKS", "WMabsDD", "U", "G", "pMSE", "PO50",
  ...
  if (is.null(plot.stat)) {
    plot.stat <- "S_pMSE"
  } else if (!(length(plot.stat) == 1 && plot.stat %in% c(
    "VW", "FT", "JSD", "SPECKS", "WMabsDD", "U", "G", "pMSE", "PO50",
...
# now make tabs of results
  ...
  if (tables == "oneway") {
    dimnames(tabs)[[1]] <- paste(X1)
  } else if (tables == "twoway") {
    dimnames(tabs)[[1]] <- paste(X1, X2, sep = ":")
  } else if (tables == "threeway") dimnames(tabs)[[1]] <- paste(X1, X2, X3, sep = ":")

  val <- rep(NA, npairs)

  for (i in 1:npairs) {
    utility.list[[i]] <- utility.tab(object, data,
      vars = pairs[, i],
...
# find worst set of variables
  ...
  for (i in 1:nworst) {
    worsttabs[[i]] <- list(
      tab.obs = utility.list[[tab.obs = worstnind[i]]]$tab.obs,
...
# calculate vars with highest scores
  ...
  if (tables == "threeway") {
    if (is.null(third.var)) {
      third.var <- names(var.scores)[1]
    } # select worst as third.var if not specified
    else {
      third.var <- names.vars[vars == third.var]
    }
  }

  if (tables == "oneway") {
    toplot <- data.frame(X1 = X1, X2 = X2, val = val)
  } else if (tables == "twoway") {
    toplot <- rbind(
      data.frame(X1 = X1, X2 = X2, val = val),
  ...
  if (is.null(plot.title)) {
    if (tables == "twoway") {
      plot.title <- bquote("Two-way utility:" ~ bold(.(plot.stat)) ~ "for pairs of variables")
    } else if (tables == "oneway") {
      plot.title <- bquote("One-way utility:" ~ bold(.(plot.stat)) ~ "for each variable")
    } else if (tables == "threeway") {
      plot.title <- {
        bquote("Three-way utility:" ~ bold(.(plot.stat)) ~ "for pairs along with" ~ bold(.(third.var)))
      }
    }
  }
  ...
  if (!is.null(max.scale)) {
    ...
    max.scale <- max(toplot$val, na.rm = TRUE)
  }
  ...
  utility.plot <- ggplot(toplot, aes(x = X2, y = X1)) +
    ...
    theme_minimal() +
    theme(
      axis.text.x = element_text(size = 10, angle = 90, hjust = 0.9, vjust = 0.2),
...
ntoc <- function(x) {
  nch <- nchar(max(x))
  res <- as.character(x)
  res <- stringr::str_pad(res, nch, "left", "0")
  return(res)
}
